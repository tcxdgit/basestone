...
def load_data(original_data = None, train_size = 0.7, m = None):
    ...
    return Xn_train[:1510], Yn_train[:1510], Xn_test[:650], Y_test[:650], min_max_scaler #1514 655
    #return Xn_train[:1530], Yn_train[:1530], Xn_test[:660], Y_test[:660], min_max_scaler #1531 662
    #return Xn_train[:1530], Yn_train[:1530], Xn_test[:660], Y_test[:660], min_max_scaler #1531 662

Xn_train, Yn_train, Xn_test, Y_test, min_max_scaler = load_data(original_data='data2.csv', train_size=0.7, m=anum)
...
model = Sequential()
model.add(LSTM(300, return_sequences=False,batch_input_shape=(10,anum,1),activation='tanh',stateful=True,consume_less='cpu',input_length=anum))
...
for i in range(40):
     model.fit(Xn_train, Yn_train, nb_epoch=1, batch_size=10, verbose=2,shuffle=False)
     model.reset_states()
#model.fit(Xn_train, Yn_train, nb_epoch=40, batch_size=10, verbose=2)
